...
/**
 * Filter results against existing article titles
 */
export function filterAgainstExisting(
  results: UnifiedSearchResult[],
  existingArticles: Array<{ title: string, urls: string[] }>,
  threshold: number = 0.4
): { unique: UnifiedSearchResult[], duplicates: string[] } {
  ...
  const unique = results.filter(result => {
    for (const existing of existingArticles) {
      // 1. URL Match (Exact)
      if (result.url && existing.urls.includes(result.url)) {
        duplicates.push(`${result.title} (URL already used in: ${existing.title})`);
        return false;
      }

      // 2. Title Similarity
      const similarity = calculateTitleSimilarity(result.title, existing.title);
      if (similarity > threshold) {
        duplicates.push(`${result.title} (similar to: ${existing.title})`);
        return false;
      }
    }
    return true;
